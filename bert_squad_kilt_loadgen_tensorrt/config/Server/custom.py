...
class CHAI(ServerGPUBaseConfig):
    system = KnownSystem.Chai
                                                                                                                                                                 # Applicable fields for this benchmark are listed below. Not all of these are necessary, and some may be defined in the BaseConfig already and inherited.    # Please see NVIDIA's submission config files for example values and which fields to keep.                                                                   # Required fields (Must be set or inherited to run):
...
class CHAI_HighAccuracy(CHAI):
    precision = "fp16"
    #server_target_qps = CHAI.server_target_qps / 2
    model_path: str = 'build/models/bert/bert_large_v1_1.onnx'
    server_target_qps: int = 950

@ConfigRegistry.register(HarnessType.Triton, AccuracyTarget.k_99, PowerSetting.MaxP)
class CHAI_Triton(CHAI):
    use_triton = True
